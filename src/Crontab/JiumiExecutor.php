...
use Jiumi\Interfaces\ServiceInterface\CrontabLogServiceInterface;
use Carbon\Carbon;
use Closure;
...
use Jiumi\Crontab\Mutex\RedisServerMutex;
use Jiumi\Crontab\Mutex\RedisTaskMutex;
use Jiumi\Crontab\Mutex\ServerMutex;
use Jiumi\Crontab\Mutex\TaskMutex;
...
class JiumiExecutor
{
     ...
     * @throws \Psr\Container\NotFoundExceptionInterface
     */
    public function execute(JiumiCrontab $crontab, bool $run = false): ?bool
    {
        if ((! $crontab instanceof JiumiCrontab || ! $crontab->getExecuteTime()) && !$run) {
            return null;
        }
        $diff = 0;
    ...
    protected function runInSingleton(JiumiCrontab $crontab, Closure $runnable): Closure
    {
    ...
    protected function runOnOneServer(JiumiCrontab $crontab, Closure $runnable): Closure
    {
        return function () use ($crontab, $runnable) {
     ...
     * @return Closure
     */
    protected function decorateRunnable(JiumiCrontab $crontab, Closure $runnable): Closure
    {
    ...
    protected function logResult(JiumiCrontab $crontab, bool $isSuccess, $result = '')
    {
        if ($this->logger) {
            if ($isSuccess) {
        ...
        $data = [
            ...
            'status' => $isSuccess ? JiumiModel::ENABLE : JiumiModel::DISABLE,
