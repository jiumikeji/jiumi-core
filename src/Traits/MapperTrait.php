...
trait MapperTrait
{
    ...
    {
        if (! config('jiumiadmin.remote_api_enabled')) {
            throw new JiumiException('系统未启用【远程通用列表查询】', 500);
        }
        /* @var $model JiumiModel */
        $model = $this->getModel();
        $query = null;
        ...
        return method_exists($this, 'handleItems')
            ? ( new JiumiCollection($this->handleItems($query->get($params['select'] ?? ['*']))) )->toArray()
