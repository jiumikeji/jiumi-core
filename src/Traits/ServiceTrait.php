...
trait ServiceTrait
{
    ...
    {
        return $value == JiumiModel::ENABLE ? $this->mapper->enable([ $id ], $filed) : $this->mapper->disable([ $id ], $filed);
    }
    ...
    {
        if (empty($dto)) {
            return container()->get(JiumiResponse::class)->error('导出未指定DTO');
        }
        ...
        return (new JiumiCollection())->export($dto, $filename, $this->mapper->getList($params), $callbackData);
    }
